        ...
        <div class="section-body row">
            ...
            if (empty($_REQUEST['searchInput'])) {
                $sqlShowProduct = "SELECT * FROM Product JOIN Productdetail ON Product.ProductId = Productdetail.ProductId
                                ...
                                ORDER BY Name";
                // echo '<div class="row">';
                // echo '<div class="page-section px-0 col">';
                // echo '<div class="section-body row">';
                // echo '<strong>KHÔNG TÌM THẤY SẢN PHẨM PHÙ HỢP!</strong>';
                // echo '</div></div></div>';
            } else {
                $search_input = $_REQUEST['searchInput'];
                $sqlShowProduct = "SELECT * FROM Product JOIN Productdetail ON Product.ProductId = Productdetail.ProductId
                                WHERE CapacityId = 1 AND Status = 1 AND (Name LIKE '%$search_input%' OR Description LIKE '%$search_input%' OR Nutrition LIKE '%$search_input%')
                                ORDER BY Name";
            }
            $tableProduct = mysqli_query($con, $sqlShowProduct);

            if (mysqli_num_rows($tableProduct) > 0) {
                ...
                while ($row = mysqli_fetch_array($tableProduct)) {
                    ...
                    <div class="col-6 col-md-4 col-lg-2 search-item">
            ...
            <?php   }
            } else echo "<div class='mt-5 mx-auto'><strong>KHÔNG TÌM THẤY SẢN PHẨM PHÙ HỢP!</strong></div>";
            ?>
        </div>
    </div>
</div>
