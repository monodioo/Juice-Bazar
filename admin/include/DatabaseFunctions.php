...
{
    $sql = 'SELECT pc.`CapacityId`, pc.`Price`, pc.`Quantity` FROM `Inventory` pc JOIN `product` p ON pc.`ProductId` = p.`ProductId` WHERE pc.`ProductId` = :ProductId ORDER BY pc.`CapacityId`';
